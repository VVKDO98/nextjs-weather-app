...
function BotInfos({ data, isMetric }) {
  return (
    <div className="w-full flex flex-row justify-between items-end">
      <div>
        <div className="flex flex-row items-center gap-2">
          <BsWind className="text-zinc-400" />
          <span>{isMetric ? `${Math.floor(data.wind.speed * 3.6)}km/h` : `${Math.floor(data.wind.speed)}m/h`}</span>
        </div>
        <div className="flex flex-row items-center gap-2">
      ...
      <div>
        <span
          className="text-6xl sm:text-8xl
                    font-light"
        >
          {isMetric ? `${Math.floor(data.main.temp - 273.15)}°c` : `${Math.floor(((data.main.temp - 273.15) * 9/5 + 32))}°f`}
        </span>
      </div>
    </div>
  );
